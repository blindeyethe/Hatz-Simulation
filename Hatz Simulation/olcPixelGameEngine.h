...
namespace olc
{
	...
	public: // DRAWING ROUTINES
		...
		// Draws a decal with 4 arbitrary point, warping the texture to look "correct"
...
namespace olc
{
	...
	void PixelGameEngine::FillTriangle(int32_t x1, int32_t y1, int32_t x2, int32_t y2, int32_t x3, int32_t y3, Pixel p)
	{
		...
		t1x = t2x = x1; y = y1;   // Starting point
		dx1 = (int)(x2 - x1);
		...
		next2:
			...
			drawline(minx, maxx, y);    // Draw line from min to max point found on the y
										// Now increase y
	...
	void PixelGameEngine::DrawPolygonDecal(olc::Decal* decal, const std::vector<olc::vf2d>& pos, const std::vector<olc::vf2d>& uv, const olc::Pixel tint)
	{
		DecalInstance di;
		di.decal = decal;
		di.point = uint32_t(pos.size());
		di.pos.resize(di.point);
		di.uv.resize(di.point);
		di.w.resize(di.point);
		di.tint.resize(di.point);
		for (uint32_t i = 0; i < di.point; i++)
		{
			di.pos[i] = { (pos[i].x * vInvScreenSize.x) * 2.0f - 1.0f, ((pos[i].y * vInvScreenSize.y) * 2.0f - 1.0f) * -1.0f };
			di.uv[i] = uv[i];
			di.tint[i] = tint;
	...
	{
		std::array<olc::vf2d, 4> point = { { {pos}, {pos.x, pos.y + size.y}, {pos + size}, {pos.x + size.x, pos.y} } };
		std::array<olc::vf2d, 4> uvs = { {{0,0},{0,0},{0,0},{0,0}} };
		std::array<olc::Pixel, 4> cols = { {col, col, col, col} };
		DrawExplicitDecal(nullptr, point.data(), uvs.data(), cols.data(), 4);
	}
...
namespace olc
{
	...
	public:
		...
		void DrawDecal(const olc::DecalInstance& decal) override
		{
			...
			for (uint32_t n = 0; n < decal.point; n++)
			{
...
namespace olc
{
	...
	public:
		...
		void DrawDecal(const olc::DecalInstance& decal) override
		{
			...
			for (uint32_t i = 0; i < decal.point; i++)
				pVertexMem[i] = { { decal.pos[i].x, decal.pos[i].y, decal.w[i] }, { decal.uv[i].x, decal.uv[i].y }, decal.tint[i] };

			locBufferData(0x8892, sizeof(locVertex) * decal.point, pVertexMem, 0x88E0);

			if (nDecalMode == DecalMode::WIREFRAME)
				glDrawArrays(GL_LINE_LOOP, 0, decal.point);
			else
				glDrawArrays(GL_TRIANGLE_FAN, 0, decal.point);
		}
